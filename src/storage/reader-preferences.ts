...
export function validatePreferences(value: unknown): ReaderPreferences {
  ...
  if (typeof data.background === "string" && ["black", "dark", "light"].includes(data.background)) result.background = data.background as ReaderPreferences["background"];
  return result;
}
